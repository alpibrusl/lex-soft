...
fn health_json(db :: Db, org :: Str, version :: Str) -> [sql, fs_read] Str {
  ...
  let counts := JObj([("agents", JInt(count(db, "agents"))), ("tenants", JInt(distinct_tenants(db))), ("accounts", JInt(count(db, "accounts"))), ("credentials", JInt(count(db, "credentials"))), ("active_credentials", JInt(count_where(db, "credentials", "WHERE revoked=0"))), ("relationships", JInt(count_where(db, "relationships", "WHERE active=1"))), ("trail_events", JInt(count(db, "events"))), ("agent_traces", JInt(count(db, "traces"))), ("approvals_pending", JInt(count_where(db, "approvals", "WHERE status='pending'"))), ("known_peer_nodes", JInt(count(db, "org_directory")))])
