...
fn edges_between(db :: Db, from_agent :: Str, to_agent :: Str) -> [sql, fs_read] Result[List[Relationship], Str] {
  let q := "SELECT id, from_agent, to_agent, role, contract_json, active FROM relationships WHERE from_agent=? AND to_agent=? AND active=1"
  let rows :: Result[List[RelRow], SqlError] := sql.query(db, q, [PStr(from_agent), PStr(to_agent)])
  match rows {
    Err(e) => Err(e.message),
    Ok(rs) => Ok(list.map(rs, fn (r :: RelRow) -> Relationship {
      parse_rel_row(r)
