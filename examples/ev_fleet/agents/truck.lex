...
fn truck_capability() -> cap.Capability {
  cap.inbound(
    "handle",
    "Accept operational messages: dispatch, charging grant/deny, status requests.",
    { title: "TruckMessage", description: "Inbound message for a truck agent.", fields: [sch.required_str("text", [])] })
}
...
fn make_tools(truck_id :: Str, tms_url :: Str, telemetry_url :: Str, logistics_url :: Str) -> List[t.Tool] {
  [
    t.define(
      "get_telemetry",
      "Get live telemetry (SoC%, odometer, speed, status) for this vehicle.",
      { title: "GetTelemetry", description: "No parameters needed.", fields: [] },
    ...
    t.define(
      "get_pending_orders",
      "Get orders pending assignment or currently assigned to this truck from TMS.",
    ...
    t.define(
      ...
      "Estimate energy (kWh) needed for a route using the logistics CAW model.",
      {
        title: "EstimateRouteEnergy",
        description: "Route energy estimation.",
        fields: [
          { name: "origin",      type: "string", required: true,  description: "Origin stop ID or address.",      constraints: [] },
          { name: "destination", type: "string", required: true,  description: "Destination stop ID or address.", constraints: [] },
        ],
      },
      fn (args :: jv.Json) -> [io, time, crypto, random, sql, fs_read, fs_write, net, concurrent, llm, proc] Result[jv.Json, Errors] {
        let origin := match jv.get_field(args, "origin")      { Some(JStr(s)) => s, _ => "" }
        let dest   := match jv.get_field(args, "destination") { Some(JStr(s)) => s, _ => "" }
    ...
    t.define(
      "report_status",
      "Report truck status to TMS (available, on_route, charging, breakdown).",
      {
        title: "ReportStatus",
...
fn system_prompt(truck_id :: Str) -> Str {
  str.concat(
    "You are autonomous truck agent ",
    str.concat(truck_id,
    ". Decisions: accept/decline loads, request charging when SoC < 20%, \
report status changes to TMS, coordinate with depots. \
Use get_telemetry for live SoC and position. \
Use get_pending_orders to check your load queue. \
Use estimate_route_energy before accepting a long haul. \
Use find_peers(intent='charging') to discover depots; send_message with \
topic='charging_request' and payload JSON including vin, soc_pct, available_minutes. \
Use find_peers(intent='dispatch') to reach your TMS providers. \
...
fn make_agent_def(db :: Db, truck_id :: Str, base_url :: Str, tms_url :: Str, telemetry_url :: Str, logistics_url :: Str, provider :: prov.Provider, model_name :: Str) -> srv.AgentDef {
  ...
  let cfg := {
    ...
    provider:      provider,
    tools:         make_tools(truck_id, tms_url, telemetry_url, logistics_url),
  }
